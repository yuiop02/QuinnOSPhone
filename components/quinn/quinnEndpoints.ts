...
const configuredLooksLocal =
  configuredBackendBaseUrl.includes('127.0.0.1') ||
  configuredBackendBaseUrl.includes('localhost');

const shouldUseConfiguredBackend =
  Boolean(configuredBackendBaseUrl) &&
  (Platform.OS === 'web' || !configuredLooksLocal);
...
const QUINN_PUBLIC_BACKEND_BASE_URL =
  shouldUseConfiguredBackend
    ? configuredBackendBaseUrl
    : useLocalWebBackend
      ? LOCAL_WEB_BACKEND_BASE_URL
